...
- DID is a first-class identity requirement; W3C DID/VC-compatible interoperability is supported without placing sensitive attributes on-chain.
...
The confidential-document extension MUST additionally demonstrate:

`Upload V1 → durable MinIO storage → SHA-256(H1) → async proof processing → Merkle root → Polygon anchor → verify V1 → upload V2 → H2 → verify V1 remains unchanged → latest returns V2`

## Product boundary

The product is an **enterprise trust/control platform**, not an NFT marketplace. The NFT is a control/ownership representation for a uniquely identifiable enterprise asset.

## Success criteria additions

- A DID is visible and cryptographically bound to the corresponding smart-account identity.
- Asset ownership can be resolved from NFT → identity/DID.
- An administrator can prove that only authorized principals can mint and initially allocate assets.
- An auditor can independently reconstruct identity, role, permission and asset lifecycle events.
- A confidential document can be verified without publishing its contents.
