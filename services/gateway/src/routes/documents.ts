...
import { putObject, getObject, getPresignedDownloadUrl } from '../minio.js';
import { documentQueue } from '../redis.js';

export const documentRoutes: FastifyPluginAsync = async (fastify: FastifyInstance) => {
  // GET /api/documents - list documents
  fastify.get('/', async (_req, _reply) => {
    const res = await query(
      `SELECT d.document_id, d.owner_address, d.title, d.latest_version_id, d.created_at,
  ...
  fastify.get<{ Params: { id: string } }>('/:id', async (req, reply) => {
    ...
    const docRes = await query(
      `SELECT document_id, owner_address, title, latest_version_id, created_at
    ...
    const versionsRes = await query(
      `SELECT v.version_id, v.seq, v.file_name, v.mime_type, v.size_bytes,
  ...
  fastify.get<{ Params: { id: string } }>('/:id/latest', async (req, reply) => {
    const { id } = req.params;

    const res = await query(
      `SELECT d.document_id, d.title, d.owner_address,
  ...
  fastify.get<{ Params: { id: string; vid: string } }>(
    '/:id/versions/:vid/download',
    async (req, reply) => {
      const { vid } = req.params;

      const res = await query(
        `SELECT minio_key, file_name, mime_type FROM document_versions WHERE version_id = $1`,
        [vid]
      );
      ...
      const url = await getPresignedDownloadUrl(config.minio.buckets.documents, minio_key, 300);
  ...
  fastify.post('/', async (req, reply) => {
    ...
    for await (const part of parts) {
      if (part.type === 'file' && part.fieldname === 'file') {
        ...
        if (part.fieldname === 'ownerAddress') ownerAddress = part.value as string;
      }
    }
    ...
    try {
      ...
      await client.query(
        `UPDATE documents SET latest_version_id = $1 WHERE document_id = $2`,
        [versionId, documentId]
      );

      // 5. Append to latest_version_history
      await client.query(
        `INSERT INTO latest_version_history (document_id, from_version, to_version, actor)
         VALUES ($1, NULL, $2, $3)`,
        [documentId, versionId, ownerAddress.toLowerCase()]
      );
      ...
      await documentQueue.add('process-version', {
        documentId,
        versionId,
        seq: 1,
        fileName,
  ...
  fastify.post<{ Params: { id: string } }>('/:id/versions', async (req, reply) => {
    ...
    try {
      ...
      const docRes = await client.query(
        `SELECT document_id, latest_version_id FROM documents WHERE document_id = $1 FOR UPDATE`,
        [documentId]
      );
      if (docRes.rows.length === 0) {
        await client.query('ROLLBACK');
      ...
      const seqRes = await client.query(
        `SELECT COALESCE(MAX(seq), 0) + 1 AS next_seq FROM document_versions WHERE document_id = $1`,
        [documentId]
      );
      ...
      await client.query(
        `INSERT INTO latest_version_history (document_id, from_version, to_version, actor)
         VALUES ($1, $2, $3, $4)`,
        [documentId, prevVersionId, newVersionId, ownerAddress.toLowerCase()]
      );
