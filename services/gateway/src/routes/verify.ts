...
export const verifyRoutes: FastifyPluginAsync = async (fastify: FastifyInstance) => {
  ...
  fastify.post<{ Params: { versionId: string } }>('/:versionId', async (req, reply) => {
    const { versionId } = req.params;

    // 1. Fetch version record + proof record + batch record from DB
    const res = await query(
      `SELECT v.version_id, v.document_id, v.sha256, v.minio_key, v.state,
    ...
    try {
      const stream = await getObject(config.minio.buckets.documents, row.minio_key);
      const chunks: Buffer[] = [];
    ...
    computedSha256 = createHash('sha256').update(fileBuffer).digest('hex');
